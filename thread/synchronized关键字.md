...
jvm采用2个字来存储对象头(如果对象是数组则会分配3个字，多出来到1个字记录的是数组长度)，其主要结构是由Mark Word和Class Metadata Address组成，其结构说明如下：
...
在Java6之前，Monitor的实现完全依赖底层系统的互斥锁来实现的。互斥锁导致一个线程如果没有获取到锁，就会进入阻塞状态，唤醒需要操作系统的协助，这就需要从用户态切换到内核态来执行，这种切换代价十分昂贵，很耗处理器性能。

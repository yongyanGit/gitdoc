...
- 当设置为 0 的时候，事务提交时不会刷新日志缓冲区。而是每秒从```log buffer```写入`os buffer`并调用`fsync()`写入到`log file on disk`中。也就是说设置为 0 时是每秒刷新写入到磁盘中的，当系统崩溃，会丢失 1 秒钟的数据。

- 当设置为 2 的时候，每次事务提交时都会把```log buffer```的数据写入到`os buffer`，但是刷到磁盘操作并不会同时进行。该模式下```mysql```会每秒执行一次 刷到磁盘。

  ```
  由系统来进行fsync操作。此时如果数据库层宕机，则不会丢失redo log，但是如果服务器宕机，这个时候文件系统中的缓存还没有fsync到磁盘文件中，这个时候就会丢失这一部分数据
  ```
